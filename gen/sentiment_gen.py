...
from gen.utils import build_alpha_record, save_records_to_db
...
def gen_sentiment_alphas():
    ...
    # 使用示例数据（替换为您的实际数据）
    data_fields = get_single_set_fields(**data_config)
    ...
    for strategy_data in data['strategies'].values():
        for name, express in strategy_data['alpha_expressions'].items():
            r = build_alpha_record(**data_config, name=name, expression=express)
            all_records.append(r)

    print(all_records[0])
    print(f"总Alpha表达式数: {len(all_records)}")

    st.dataframe(all_records)

    if st.button("保存到数据库"):
        save_records_to_db(**data_config, new_alphas=all_records)
